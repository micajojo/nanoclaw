...
- **Short turn (≤2 quick tool calls):** Skip `send_message` entirely — wrap your response in `<message>` blocks in your final output. Do **not** call `send_message` and then also include the same content in a `<message>` block; that sends the message twice.
