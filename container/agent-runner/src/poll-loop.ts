...
export async function runPollLoop(config: PollLoopConfig): Promise<void> {
  ...
  while (true) {
    ...
    try {
      ...
      // Stale/corrupt continuation recovery: clear the stored continuation
      // and retry immediately with a fresh session rather than surfacing an
      // error to the user. One retry only — if the fresh query also fails,
      // fall through to the error-write path.
      if (continuation && config.provider.isSessionInvalid(err)) {
        log(`Stale session detected (${continuation}) — retrying with fresh session`);
        continuation = undefined;
        clearContinuation(config.providerName);
        try {
          ...
          log(`Completed ${ids.length} message(s) (after stale-session retry)`);
          continue;
        } catch (retryErr) {
          const retryMsg = retryErr instanceof Error ? retryErr.message : String(retryErr);
          log(`Retry after stale session also failed: ${retryMsg}`);
          // Fall through to error-write below using the retry error message.
          writeMessageOut({
            id: generateId(),
            kind: 'chat',
            platform_id: routing.platformId,
